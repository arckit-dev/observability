...
```bash
git clone git@github.com:arckit-dev/observability.git
cd observability
```
...
| Command | Description |
|---------|-------------|
| `pnpm build` | Build the library with [tsdown](https://tsdown.dev/) (ESM + CJS + declarations). |
...
- [tsdown](https://tsdown.dev/): fast library bundler powered by Rolldown (Rust)
...
| `CODECOV_TOKEN` | Codecov upload token | [app.codecov.io](https://app.codecov.io/gh/arckit-dev/observability/config/general) > Repository upload token |

### Workflows

| Workflow | Trigger | Description |
|----------|---------|-------------|
| [CI](.github/workflows/ci.yml) | Push to `main` / PR | Lint, type-check, test, build, package quality checks, coverage upload to [Codecov](https://codecov.io/gh/arckit-dev/observability) |
